...
// Define interfaces for the row and props
interface Row {
  id: string; // Adjust this type based on the actual structure of your row data
  para: string;
  criterion: number; // Assuming this is a number
  value: number | string; // Can be a number or a string
  type: number; // Assuming this is a number (0, 1, 2)
}

interface TableProps {
  rows: Row[]; // Array of rows
  deleteRow: (idx: number) => void; // Function to delete a row
  editRow: (idx: number) => void; // Function to edit a row
}

export const Table: React.FC<TableProps> = ({ rows, deleteRow, editRow }) => {
  // Filter fields, assuming fields are used somewhere in your component
  Object.keys(Object.values(dataJSON)[0]).filter((item: string) => !(item.startsWith("delta_")));
